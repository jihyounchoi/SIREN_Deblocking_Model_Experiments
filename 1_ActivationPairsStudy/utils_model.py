...
def set_dir(params_json):
    # Create the base directory if it doesn't exist already
    if not os.path.exists(params_json['base_dir']):
        os.makedirs(params_json['base_dir'])

    # Create a text file that introduces the parameter settings in the base directory
    with open(os.path.join(params_json['base_dir'], 'parameter_settings.txt'), 'w') as f:
        current_time = datetime.datetime.now()
        current_time_str = current_time.strftime("%Y-%m-%d %H:%M:%S")
        
        f.write("Current Time : {}\n".format(current_time_str))
        f.write('Activation Function : {}\n'.format(params_json['activation_string']))
        f.write('Omega : {}\n'.format(params_json['omega_string']))
        f.write('Residual Layers : {}\n'.format(params_json['residual_string']))
...
def parameter_printer(params_json):
    ...
    print(f"Activation Function : {params_json['activation_string']}")
    print(f"Omega : {params_json['omega_string']}")
    print(f"Residual Layers : {params_json['residual_string']}")
...
def write_successed_msg(params_json):
    ...
    msg_body += f"Omega: {params_json['omega_string']}\n"
    msg_body += f"Residual Layers: {params_json['residual_string']}\n"
...
def write_error_msg(params_json, error):
    ...
    f"- Activation Function: {params_json['activation_string']}\n"\
    f"- Omega: {params_json['omega_string']}\n"\
    f"- Residual Layers: {params_json['residual_string']}\n"\
