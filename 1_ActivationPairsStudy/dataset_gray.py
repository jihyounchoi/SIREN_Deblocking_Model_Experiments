...
class DatasetJPEG(data.Dataset):
    ...
    def __init__(self, n_channels=1, patch_size=32, dataset_path = None, phase = 'train', qf_candidates : list = [10*i for i in range(1, 10)]):
        super(DatasetJPEG, self).__init__()
        self.n_channels = n_channels if n_channels else 1  # original : 3
        self.patch_size = patch_size if patch_size else 32  # original : 64
...
if __name__ == '__main__':
    ...
    dataset_test = DatasetJPEG(n_channels = 1, patch_size = 64, dataset_path='../Dataset/testset/LIVE1_gray', phase = 'test', qf_candidates=[30])
