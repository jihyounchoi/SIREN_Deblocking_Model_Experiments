...
def main(params_json):
    ...
    if '_' in model_: # It's not recommended to use eval function
        model = eval(f"{model_.split('_')[0]}(omega = {int(model_.split('_')[1])})")
    else:
        model = eval(f"{model_}()")
...
if __name__ == '__main__':
    ...
    for i, params_json in enumerate(config['param_settings']):
        ...
        # except:
        #     msg_subject = f'Parameter Setting no.{i} Terminated by Error'
        #     msg_body = json.dumps(params_json)
        #     send_email(subject=msg_subject, body=msg_body)
        
        print(f'Finish {i + 1}th setting at {datetime.now()}')
        print('#####################################################\n\n\n')
        
        msg_subject = f'Setting {i + 1} of finished Successfully'
        msg_body = json.dumps(params_json)
        send_email(subject = msg_subject, body = msg_body)
