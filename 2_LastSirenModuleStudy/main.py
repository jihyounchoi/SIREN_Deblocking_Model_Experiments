...
def main(params_json):
    ...
    with open(f"{base_dir}/PERFORMANCE.txt", "w") as f:
        ...
        dataset_train = DatasetJPEG(patch_size=patch_size, dataset_path=datadir_train, phase='train', qf_candidates=qf_candidates)
        dataset_val = DatasetJPEG(patch_size=patch_size, dataset_path=datadir_val, phase='val', qf_candidates=qf_candidates)
        ...
        model = CustomNetwork(activation_string=activation_string, omega_string=omega_string, residual_string=residual_string, global_skip=False)
        model.to(device)
